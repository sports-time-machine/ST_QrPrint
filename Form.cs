    ...
    public partial class FormQr : Form
    {
        ...
        private void pd_PrintPage(PrintType print_type, object sender, PrintPageEventArgs e)
        {
            ...
            for (int y = 0; y < H; ++y)
            {
                for (int x = 0; x < W; ++x, ++i)
                {
                    var fullId = prefix + ids[i];
                    var showId = (showname != null) ? showname[i] : GetShortId(ids[i]);

                    const int qrWidth = 3;
                    var textWidth = (int)g.MeasureString(showId, font).Width;

                    var dx = x * cel_width + margin_h;
                    var dy = y * cel_height + margin_v;

                    barcode.QRWriteBar(fullId,
                        to_inch * (dx + 2),
                        to_inch * (dy + 2),
                        qrWidth, e.Graphics);
#if false
                    ...
                    barcode.QRWriteBar(fullId,
                        ...
                        to_inch * (dy + 2),
                        qrWidth, e.Graphics);
                    g.DrawString(showId, font, Brushes.Black,
                        new PointF(
